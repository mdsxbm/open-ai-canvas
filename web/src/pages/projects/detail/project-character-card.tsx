...
import { CharacterHealthBadge } from "./character-health-badge";
            ...
            <AssetLibraryCardMedia className="relative aspect-[3/2] overflow-hidden bg-foreground/[.045]">
                ...
                <div className="absolute inset-x-2 top-2 flex items-center justify-between gap-2">
                    ...
                    <span className="flex min-w-0 items-center gap-1.5">
                        <span className="truncate rounded bg-black/60 px-1.5 py-0.5 text-[var(--fs-micro)] text-white">{readinessLabel}</span>
                        {/* 主角模式健康度徽章（spec Task 13）：分数 + 绿/黄/红分层，<70 时 tooltip 给补图建议。 */}
                        {character ? <CharacterHealthBadge character={character} /> : null}
                    </span>
                </div>
