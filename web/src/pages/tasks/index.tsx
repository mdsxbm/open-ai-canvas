...
type TaskKindFilter = "all" | "text" | "image" | "video" | "redraw";
            ...
            <WorkspacePage grid className="library-page task-library-page">
                <div className="studio-band">
                    ...
                    >
                        ...
                        <Select className="w-full sm:w-32" value={kindFilter} onChange={(value) => { setKindFilter(value as TaskKindFilter); setPage(1); }} options={[{ label: "全部类型", value: "all" }, { label: "文本", value: "text" }, { label: "图片", value: "image" }, { label: "视频", value: "video" }, { label: "风格转绘", value: "redraw" }]} />
...
function taskEmptyState(status: TaskStatusFilter) {
    if (status === "all") return { title: "开机指令尚未发出", description: "新提交的生成会在这里显示状态和实时进度。" };
    if (status === "active") return { title: "没有正在拍摄的镜头", description: "新提交的生成会在这里显示排队状态和实时进度。" };
    if (status === "succeeded") return { title: "成片尚未入仓", description: "生成成功后，结果预览和执行记录会保留在这里。" };
    return { title: "没有 NG 或撤下的镜头", description: "失败或取消的生成会出现在这里，并提供原因和可用操作。" };
}
