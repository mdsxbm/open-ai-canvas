...
import { createAdminChannelModel, deleteAdminChannelModel, fetchAdminChannelModels, listAdminChannelModels, testAdminChannelModel, updateAdminChannelModel, type ChannelModel, type ChannelModelMutation, type ChannelModelPriceTier } from "@/services/api/wallet";
...
type TierBillingMode = ChannelModelPriceTier["billingMode"];
...
// 常见 provider payload 字段名提示，帮助管理员配置 bucketKeyFields。
// 不同模型实际字段名可能不同，最终以管理员按上游 payload 字段名填写为准。
const BUCKET_FIELD_SUGGESTIONS = ["duration", "videoSeconds", "vquality", "resolution", "size", "model"];
    ...
    const save = async () => {
        ...
        try {
            const payload = {
				...
				priceTiers: values.priceTiers.map((tier): Exclude<ChannelModelMutation["priceTiers"], undefined>[number] => ({
					...
					billingMode: tier.billingMode as TierBillingMode,
                ...
                capabilityConfig,
            } satisfies ChannelModelMutation;
...
function legacyPriceTierToForm(item: ChannelModel): PriceTierFormValues {
    // upstream 顶层 ChannelModel.billingMode 仍然保留 per_video_bucket 做兼容展示，
    // 但写入只走 priceTiers 合同，取最大公共子集：fixed_request | per_second | token，
    // 否则回落到 fixed_request 占位，等管理员在 UI 中选实际模式后保存。
    const safeMode: TierBillingMode = item.billingMode === "fixed_request" || item.billingMode === "per_second" || item.billingMode === "token"
        ? item.billingMode
        : "fixed_request";
    return {
		operation: "*", quality: "*", size: "*", resolution: "*", videoSeconds: 0, imageCount: 0, providerModelKey: item.providerModelKey || "", billingMode: safeMode,
