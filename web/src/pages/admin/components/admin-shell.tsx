...
const adminNavigation: Array<{ label: string; items: AdminNavigationItem[] }> = [
    ...
    },
    {
        ...
        items: [
            ...
            { path: "/admin/invite-codes", label: "邀请码", description: "幕山攀登计划 · 批量生成与核销查看", icon: <Mountain className="size-4" /> },
            { path: "/admin/activities", label: "活动赛事", description: "幕山攀登计划 · 创建与查看活动", icon: <Trophy className="size-4" /> },
            { path: "/admin/content-audit", label: "内容审核", description: "文本 / 图片 / 视频合规队列", icon: <ScanSearch className="size-4" /> },
