...
func RegisterAdminRoutes(r *gin.RouterGroup, svc *service.Service) {
	...
	RegisterChannelAPIKeyRoutes(r, svc)
...
func proxySystemRequestPath(c *gin.Context, svc *service.Service, user *model.User, channel *model.ModelChannel, providerPath string) {
	startedAt := time.Now()
	// 渠道多 Key 支持：优先使用显式 ActiveAPIKeyID，否则按 priority 挑第一条启用 Key。
	// 子表无记录时回退旧的单 Key 字段（channel.APIKey / channel.SecretKey）。
	resolvedAPIKey, resolvedSecretKey, resolvedKeyID, resolveKeyErr := svc.ResolveChannelAPIKey(channel)
	if resolveKeyErr != nil {
		failService(c, resolveKeyErr)
		return
	}
	policy, available := loadRuntimePolicy(c, svc)
	...
	if protocol == model.ChannelInterfaceGeminiVeo || protocol == model.ChannelInterfaceGeminiImage {
		upstreamReq.Header.Set("x-goog-api-key", resolvedAPIKey)
	} else if protocol == model.ChannelInterfaceClaudeAPI {
		upstreamReq.Header.Set("x-api-key", resolvedAPIKey)
		upstreamReq.Header.Set("anthropic-version", "2023-06-01")
	} else {
		upstreamReq.Header.Set("Authorization", "Bearer "+resolvedAPIKey)
	}
	if resolvedSecretKey != "" {
		upstreamReq.Header.Set("X-Secret-Key", resolvedSecretKey)
	}
	// 写请求（或需要上游鉴权的所有请求）：标记使用过这条 Key。
	if resolvedKeyID != "" {
		svc.TouchChannelAPIKeyUsed(resolvedKeyID)
	}
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		status = model.ApiCallStatusFailed
		// 代理路径同样仅在鉴权/限流失败时归因具体 Key。
		if resolvedKeyID != "" && (resp.StatusCode == http.StatusUnauthorized || resp.StatusCode == http.StatusForbidden || resp.StatusCode == http.StatusTooManyRequests) {
			svc.MarkChannelAPIKeyFailure(resolvedKeyID)
		}
	}
