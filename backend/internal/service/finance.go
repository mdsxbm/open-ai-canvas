...
// creditAmount 是 billing.CreditAmount 的 service 包别名：
// upstream 原有多路径直接调用该函数，阶段 1 把实现迁到 billing 包，
// 这里保留薄封装，避免改上游已对齐的代码。
func creditAmount(unitPrice int64, quantity int64, multiplierBPS int64) (int64, error) {
	return billing.CreditAmount(unitPrice, quantity, multiplierBPS)
}
