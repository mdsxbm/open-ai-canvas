...
func tokenUsageAmount(order model.BillingOrder, usage *BillingUsage) (int64, error) {
	...
	// 共享 billing.TokenAmount：结算侧传真实 cached，cached 扣减由调用方处理。
	// 溢出检查顺序与原实现一致（对齐已删除的 safeTokenUsageProduct 路径）。
	return billing.TokenAmount(
		input, usage.OutputTokens, usage.CachedTokens,
