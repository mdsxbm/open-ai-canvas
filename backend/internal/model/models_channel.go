...
type ModelChannel struct {
	...
	// ActiveAPIKeyID 是多 Key 渠道当前显式锁定的 ChannelAPIKey 子记录 ID；
	// 为空表示按 priority 升序选第一把启用 Key。保留旧 APIKey/SecretKey 作兼容回退。
...
// 选择策略：priority 升序 + enabled 过滤 + ActiveAPIKeyID 显式锁定；
...
type ChannelAPIKey struct {
	...
	APIKeySuffix string         `json:"apiKeySuffix" gorm:"size:16"` // 末 4 位，供 UI 展示，明文 Key 不出后端
